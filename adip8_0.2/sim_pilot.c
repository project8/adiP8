...
void tracking_loop(char *filename)
{
  ...
  if (f_run == (FILE *) 0) {     // disk error handling
    ...
    if (particle.calc_order == 1) {
      printf("only ExB drift\n");
    }
    if (particle.calc_order == 2) {
      printf("only curvature and gradient drift\n");
    }
    if (particle.calc_order > 2) {
      printf("no drift\n");
    }
  ...
  while (fscanf(f_run, "%d %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", &repeat_number, &particle.start_pos[0], &particle.start_pos[1], &particle.start_pos[2], &particle.e_start, &particle.starting_theta, &particle.starting_phi, &particle.mass, &particle.charge, &particle.phase) != EOF) {  // read data out of file until file ends
    ...
    sprintf(set_filename, "%s.set%d", filename, particle_count);
    ...
    for (repeated = 1; repeated <= repeat_number; repeated++) {
      ...
      sprintf(track_filename, "%s.track%d", filename, particle_count);  // data output to file with extension .track
      ...
      printf("Startparameters x=%.3f y=%.3f z=%.3f E=%.1f theta=%.1f\n", particle.start_pos[0], particle.start_pos[1], particle.start_pos[2], particle.e_start, particle.starting_theta);
      ...
      fprintf(f_set, "%.3f  %.3f  %.3f  %.3f  %.3f  %.3f\n",
              particle.position[0], particle.e_start, particle.e_kin, particle.e_para, particle.e_perp, asin(sqrt(particle.e_perp / particle.e_kin)) * 180 / M_PI);
                  ...
                  //parameters of the stored electrons
{
  ...
  if (f_tvol == (FILE *) 0) {
    ...
    if (f_tlog == (FILE *) 0) {
      ...
      if ((parameter.trap_y_plane == 1) && (parameter.trap_z_plane == 2)) {
      ...
      if (parameter.trap_start_x < parameter.trap_stop_x) {  // if start left of stop exchange values
      ...
      for (position[0] = startx; position[0] >= stopx; position[0] += stepx) {
        ...
        do {
          ...
          if (e_tag == 0) {
            y_max = y_max + parameter.el_mm_unit / 10.;
          }
        ...
        while ((position[parameter.trap_y_plane] <= parameter.trap_stop_y)) {
          // y-direction loop
          for (energy = parameter.trap_energy_start; energy >= parameter.trap_energy_end; energy = energy * 0.5) { // energy loop
            fprintf(f_tvol, "%f %f %f ", position[0], position[parameter.trap_y_plane], energy);
            ...
            do {                //theta loop
              ...
              if (sqrt(position[parameter.trap_y_plane] * position[parameter.trap_y_plane]) <= y_limit) {
          ...
          position[parameter.trap_y_plane] = position[parameter.trap_y_plane] + parameter.trap_step_y;
...
void scan_field_gradient(char *filename)
{
  ...
  for (x = 0; x <= xmax; x++) {
    for (y = 0; y <= ymax; y++) {
      ...
      if (number % 100 == 0) {
        cout << "  writing: " << (int) (100. * (float) number / (float) (xmax * ymax)) << " %   \r" << flush;
      }

      fprintf(f_pot, "%e %e %e %e %e %e %e %d\n", pos[0], pos[1], pos[2], epot, evec[0], evec[1], evec[2], e_tag);
    }
    fprintf(f_pot, "\n");
  }
  cout << "    DONE.         " << endl << endl;
...
void scan_Bfield(char *filename)
{
  ...
  for (x = 0; x <= (int) parameter.spec_out; x++) {
    for (y = 0; y <= (int) parameter.max_radius; y++) {
      ...
      if (number % 100 == 0) {
        cout << "  writing: " << (int) (100. * (float) number / (float) (parameter.spec_out * parameter.max_radius)) << " %   \r" << flush;
      }

      fprintf(f_bvec, "%e %e %e %e %e %e %e\n", pos[0], pos[1], pos[2], bvec[0], bvec[1], bvec[2], sqrt(bvec[0] * bvec[0] + bvec[1] * bvec[1] + bvec[2] * bvec[2]));
    }
    fprintf(f_bvec, "\n");
    fflush(f_bvec);
  }
...
void mc_tracking(char *filename)
{
  ...
  while (fscanf(f_run, "%d %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", &repeat_number, &particle.start_pos[0], &particle.start_pos[1], &particle.start_pos[2], &particle.e_start, &particle.starting_theta, &particle.starting_phi, &particle.mass, &particle.charge, &particle.phase) != EOF) {  // read data out of file until file ends
    ...
    sprintf(set_filename, "%s.mc%d", filename, particle_count);
    f_set = fopen(set_filename, "w");
    ...
    for (repeated = 1; repeated <= repeat_number; repeated++) {
      ...
      if (particle.mirrors >= 2) {
        ...
        cout << "Particle NOT trapped! -> skipped" << endl;
      }
