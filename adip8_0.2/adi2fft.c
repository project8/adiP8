...
*/
  ...
  if (parameter.fft_on==1) transform = kTRUE;
  ...
  if (parameter.rad_atten == 1 ) atten = attenR+attenG;
  tl_data.att = atten;
  ...
  if (!refCo==0) cout << " Reflected wave attenuation: " << exp(-atten*abs(3*x_ant)) << endl;
 ...
 //1- file reads in tracking information, loop over tracked points, 
  while (runfile >> repeat >> xi >> yi >> zi >> ekin >> thetai >> phii >> mass >> charge >> phasei)
  {
    ...
    if (addNoise) in_n = (double*) fftw_malloc(sizeof(double) * N);
    if (addNoise) out_n = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * N);
    ...
    if (addNoise) nenergyf = (double*) fftw_malloc(sizeof(double) * (N/2+1));
    if (addNoise) npowerf = (double*) fftw_malloc(sizeof(double) * (N/2+1));
    if (addNoise) nESD = (double*) fftw_malloc(sizeof(double) * (N/2+1));
    ...
    if (shift==1) for_T = inter_T+dir*(last_simX[0]+1e-4*last_simV[0]*(inter_T-last_simT))/c; 
    ...
    if (shift==1) bk_T = inter_T-t_del-dir*(last_simX[0]+1e-4*last_simV[0]*(inter_T-last_simT))/c; 
    double inter_R, drdt;
    ...
    while (trackfile >>  next_simX[0] >> next_simX[1] >> next_simX[2] >> next_simR >> next_ep >> next_ek >> next_epar >> next_eperp >> next_egain >> next_eloss >> next_b >> next_simPhase >> next_simT >> next_scatt >> next_wq >> next_simV[0] >> next_simV[1] >> next_simV[2] >> next_simOm)
    {
      ...
      while ((for_T< simT)) {//interpolate between simulated steps
        ...
        for (int j=0; j<3; j++) inter_X[j]=last_simX[j]+dxdt[j]*(for_T-last_simT);
        //guiding center velocity in m/s
        for (int j=0; j<3; j++) inter_V[j]=last_simV[j]+dvdt[j]*(for_T-last_simT);
        ...
        inter_T = (itf+1)*tstep;
        for_T = inter_T;
        if (shift == 1) for_T = inter_T +dir*inter_X[0]/c; 
        if (itf >= N) break;
        if (inter_T> tmax) break;
        if (status > 0) break;
      ...
      while ((bk_T< simT)) {//interpolate between simulated steps
        ...
        //velocity in m/s
        for (int j=0; j<3; j++) inter_V[j]=last_simV[j]+dvdt[j]*(bk_T-last_simT);
        if (abs(inter_X[0])>abs(x_ant)) 
        {
          cout << "Warning! Passed Antenna! Exiting Now!" << endl;//passed antenna
          status = 1;
        ...
        inter_T = (itb+1)*tstep;
        bk_T = inter_T-t_del;
        if (shift==1) bk_T = inter_T-t_del-dir*inter_X[0]/c; 
        if (itb >= N) break;
        if (inter_T> tmax) break;
        if (status > 0) break;
        //if (zin[it] > 0 && yin[it] > 0 && it > 1000) break;//for tracking half orbit of offset wires
      ...
      //reset last values 
      if (last_ek - ek > 1.0) nscatters++ ; 
      last_simT = simT;
      last_simR = simR;
      last_simPhase = simPhase;
      last_simOm = simOm;
      for (int j=0; j<3; j++) last_simX[j] = simX[j];
      //for (int j=0; j<3; j++) last_simV[j] = dxdt[j];//calculat from position
      for (int j=0; j<3; j++) last_simV[j] = simV[j];
      last_ek = ek;
      ...
      simOm = next_simOm;
      for (int j=0; j<3; j++) simX[j] = next_simX[j];
      for (int j=0; j<3; j++) simV[j] = next_simV[j];
      ek = next_ek;
      b = next_b;
      eloss = next_eloss;
      ep = next_ep; epar = next_epar; eperp = next_eperp;
      egain = next_egain; scatt = next_scatt; wq = next_wq;
    }
    ...
    for ( it = 0; it< max; it++ ) {
      ...
      anti.Ef_bk = 0;
      if( n_del<=it) anti.Ef_bk = refCo*in_bk[it-n_del];//coeff of efield, fN.cm/C
      ...
      if (addNoise) anti.nEf = sqrt(tl_data.Zw/tl_data.Zc)*1e+17*anti.vtot;//fN.cm/C
      if (addNoise) in_n[it] = anti.nEf;
    ...
    if (transform ) {
      ...
      */ 
      ...
      for (int j=0;j<maxf;j++)
      {
        ...
        if (addNoise) nenergyf[j] = 1e-2/tl_data.Zw*tstep/max*(pow(out_n[j][0],2)+pow(out_n[j][1],2));
        ...
        if (addNoise) npowerf[j] = 1e4/tl_data.Zw/max/max*(pow(out_n[j][0],2)+pow(out_n[j][1],2));
        ...
        if (addNoise) nESD[j] = 1e-8/tl_data.Zw*(pow(tstep*out_n[j][0],2)+pow(tstep*out_n[j][1],2));
      }
      ...
      for (int j=maxf;j<max;j++)//for k=1...maxf-1(excluding DC and nyquist)
      {
        ...
        if (addNoise) nenergyf[max-j] += 1e-2/tl_data.Zw*tstep/max*(pow(out_n[max-j][0],2)+pow(out_n[max-j][1],2));
        ...
        if (addNoise) npowerf[max-j] += 1e4/tl_data.Zw/max/max*(pow(out_n[max-j][0],2)+pow(out_n[max-j][1],2));
        ...
        if (addNoise) nESD[max-j] += 1e-8/tl_data.Zw*(pow(tstep*out_n[max-j][0],2)+pow(tstep*out_n[max-j][1],2));
      }
