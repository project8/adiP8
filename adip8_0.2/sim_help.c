...
{
  printf("%s: ", nr);
  printf("%f  %f  %f  %f", (*pdata).e_para, (*pdata).e_perp, (*pdata).e_accel, (*pdata).e_syncro);
  printf("\n");
}
     ...
     // stores data to the scatter log file "adi_scatter.log"
{
  ...
  if (scatlog_set) {
    ...
    fprintf(f_log, "%d %8.3f %6.2f %10.3f  %e  %8.3f", process, e_loss, angle, (*particle).e_kin, (*particle).time_of_flight, e_loss - (*particle).e_pot);
    fprintf(f_log, " %8.3f %8.3f", (*particle).position[0], sqrt((*particle).position[1] * (*particle).position[1] + (*particle).position[2] * (*particle).position[2]));
    fprintf(f_log, "\n");

    fclose(f_log);
  } else {
    cout << endl << "ERROR: scatlog filename not set!" << endl << endl;
  }
}
...
void sim_help_show_run_file(char *filename)
{
  ...
  if (f_run == (FILE *) 0) {     // disk error handling
    ...
    if (parameter.calc_order == 0) {
      printf("all drifts\n");
    }
    if (parameter.calc_order == 1) {
      printf("only ExB drift\n");
    }
    if (parameter.calc_order == 2) {
  ...
  while (fscanf(f_run, "%d %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", &repeat_number, &start_pos[0], &start_pos[1], &start_pos[2], &e_start, &starting_theta, &starting_phi, &mass, &charge, &phase) != EOF) {  // read data out of RUN-file until EOF
    printf("%8d %9.3f %9.3f %9.3f %7.1f %4.1f %4.1f %3.1f %3.1f %4.1f\n", repeat_number, start_pos[0], start_pos[1], start_pos[2], e_start, starting_theta, starting_phi, mass, charge, phase);
  }
  printf("----------------------------------------------------------------\n");
...
{                               // save one datablock to harddisc
  if (((*particle).position[0] >= -(*particle).max_step_length) && ((*particle).position[0] <= (*particle).max_step_length)) {
    sim_help_save_data_count = SAVE_EVERY;  // force data saved around analysing plane
  }
  if (((*particle).position[0] >= (SPEC_OUT - (*particle).max_step_length * 10)) || ((*particle).position[0] <= (SPEC_IN + (*particle).max_step_length * 10))) {
    sim_help_save_data_count = SAVE_EVERY;  // force data saved at source and detector
  }
  if (sim_help_save_data_count == SAVE_EVERY) {
    sim_help_save_data_count = 0;
    fprintf(f_fly, "%1.9f %1.9f %1.9f ", (*particle).position[0],  // 1
            (*particle).position[1],  // 2
            (*particle).position[2]);  // 3
    fprintf(f_fly, "%1.9f ", (*particle).cyclrad);  // 4
    fprintf(f_fly, "%1.9f %1.9f ", (*particle).e_pot,  // 5
            (*particle).e_kin); // 6
    fprintf(f_fly, "%1.9f %1.9f ", (*particle).e_para,  // 7
            (*particle).e_perp);  // 8
    fprintf(f_fly, "%1.9e %1.9e ", (*particle).e_accel_sum,  // 9
            (*particle).e_syncro_sum);  // 10
    fprintf(f_fly, "%1.9f %1.15e %1.15e %1.12e %e ", (*particle).b_value,  // 11
            (*particle).phase,  // 12
            (*particle).time_of_flight,  // 13
            scatter_get_ratio(),  // 14
            get_wq((*particle).e_kin));  // 15
    fprintf(f_fly, "%1.9f %1.9f %1.9f %1.9e ", (*particle).v_para[0],  // 16 
