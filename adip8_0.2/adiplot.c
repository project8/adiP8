...
int main(int argc,char *argv[])
{
  ...
  for ( Int_t event = 1; event <=nEvents; event++ ) {
    ...
    if (refCo == 1) tSize = 1.3e-5;//in seconds, 4xfree space energy loss
    ...
    //spectrogram 
    TH2F *h2 = new TH2F(Form("fvst_%d",event), "Spectrogram;Time (ms);Frequency (MHz); Power (fW)",nX, -0.5*tSize*1e3, (nX-0.5)*tSize*1e3, nY, -0.5*delf*1e-6, (nY-0.5)*delf*1e-6); 
    //noise in single PS
    ...
    for(int i=0;i<nX;i++)
    {
      ...
      for (int j=Ntot/2+1;j<Ntot;j++)
      {
        ...
        if (powerf[Ntot-j] > maxP) maxP = powerf[Ntot-j];
      }
      if (signalFT) {
