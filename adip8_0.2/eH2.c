...
//
  ...
  int j;
  double K2,KK,fE,kej,ki,kf,Rex,arg,arctg; int i;
  double st1,st2;
//
...
// Del: in m^2/steradian
  ...
  {
    gam=1.+T/(clight*clight);   // relativistic correction factor
    K2=2.*T*(1.+gam)*(1.-c); if(K2<0.) K2=1.e-30;
...
// model.
    if(K<3.)
    { i=(int)(K/0.1); Ki=i*0.1;
      CelK=Cel[i]+(K-Ki)/0.1*(Cel[i+1]-Cel[i]);}
    else if(K>=3. && K < 5.)
    { i=(int)(30+(K-3.)/0.2); Ki=3.+(i-30)*0.2;
      CelK=Cel[i]+(K-Ki)/0.2*(Cel[i+1]-Cel[i]);}
    else if(K>=5. && K < 9.49)
    { i=(int)(40+(K-5.)/0.5); Ki=5.+(i-40)*0.5;
...
//
  ...
  if(E>=100.)
  {
    K2=4.*T*(1.-EE/(2.*T)-sqrt(1.-EE/T)*c); if(K2<1.e-9) K2=1.e-9;
    K=sqrt(K2);               // momentum transfer
...
// Dinel: in m2/steradian
  ...
  if(E<16.)
    return Dexc(E,c);
  T=E/27.2;
  K2=4.*T*(1.-Ei/(2.*T)-sqrt(1.-Ei/T)*c); if(K2<1.e-9) K2=1.e-9;
  K=sqrt(K2);        // momentum transfer
  st1=1.+K2/4.;
...
double lagrange(int n,double *xn,double *fn,double x)
{
  ...
  f=0.;
  for(j=0;j<n;j++)
  { for(i=0;i<n;i++) { a[i]=x-xn[i]; b[i]=xn[j]-xn[i];}
    a[j]=b[j]=aa=bb=1.;
    for(i=0;i<n;i++) { aa=aa*a[i]; bb=bb*b[i];}
    f+=fn[j]*aa/bb;
  }
  return f;
}
