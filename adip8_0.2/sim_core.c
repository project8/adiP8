     ...
     // if particle gets scattered
{
  ...
  if ((rot0[0] == 0.) || (rot0[1] == 0.) || (rot0[2] == 0.))
    {
      if (rot0[0] == 0.) trans1[0] = 1.;
      else if (rot0[1] == 0.) trans1[1] = 1.;
      else if (rot0[2] == 0.) trans1[2] = 1.;
    }
  ...
  vector_times_scalar(trans1,
		      sqrt((*particle).e_kin)
		      *tan((M_PI/180.)
			   *scatter_get_angle())
		      /absvalue(trans1), trans1);
  //  printf("trans1': %f %f %f\n",trans1[0],trans1[1],trans1[2]);
  ...
  if ((*particle).delta_tof > 0.)
    {
      ...
      p_syncrad = (2*electron_charge*electron_charge*lightspeed*
		   (*particle).delta_e_para*(*particle).delta_e_para)
...
{ // this function calculates the para syncrotron radiation of particle in eV
  ...
  if ((*particle).e_cycl > 0.)
    {
      cycrad = sqrt((*particle).e_cycl*(*particle).mass*
		    (get_gamma(particle)+1.))
	/(3000000.*(*particle).b_value); 
      ...
      p_syncrad = (2*electron_charge*electron_charge*lightspeed
		   *beta*beta*beta*beta
		   *gamma*gamma*gamma*gamma)/(3*cycrad*cycrad);
...
{ // this function gathers and includes all calculated energy losses to current particle
  ...
  if ((*particle).e_perp < 0.) (*particle).e_perp = 0.;
  if ((*particle).e_cycl < 0.) (*particle).e_cycl = 0.;
     ...
     // this is a real step forward
{
  ...
  (*particle).time_of_flight = (*particle).time_of_flight 
                               + (*particle).delta_tof*1000000.;
  ...
  if (!scatter_check_event())
	...
	default :
      ...
      sim_help_store_scatter_data(particle,scatter_get_eloss(),
				  scatter_get_angle(),scatter_get_process());
...
void mag_drift(struct particle_data* particle)
{
  ...
  for (komp = 0; komp <= 2; komp++)
    {	// this is a simple gradient calculation for Bfield value
  ...
  vector_times_scalar((*particle).b_vec_old,
		      1./absvalue((*particle).b_vec_old),
		      b_unitary_old);
  vector_times_scalar((*particle).b_vec,
		      1./absvalue((*particle).b_vec),
		      b_unitary);
...
{ // this routine calculates the energies, perpendicular part and the parallel part
  ...
  (*particle).e_curv = gamma*gamma*(*particle).mass*
                       absvalue((*particle).rxb_vel)*
                       absvalue((*particle).rxb_vel)
                       /(Clight*Clight*(gamma+1));

  (*particle).e_grad = gamma*gamma*(*particle).mass*
                       absvalue((*particle).dbxb_vel)*
                       absvalue((*particle).dbxb_vel)
                        /(Clight*Clight*(gamma+1));

  (*particle).e_ExB  = gamma*gamma*(*particle).mass*
                       absvalue((*particle).exb_vel)*
                       absvalue((*particle).exb_vel)
                       /(Clight*Clight*(gamma+1));

  delta_e_perp = (*particle).e_perp*
    (((*particle).b_value/(*particle).b_value_old)-1.);
...
void calc_velocities(struct particle_data* particle)
{
  ...
  (*particle).v_perp_value = sqrt((get_gamma(particle) + 1.)*(*particle).e_perp/
    ((*particle).mass*get_gamma(particle)*get_gamma(particle)))*Clight; // new relativistic version
...
int single_track_trap(struct particle_data* particle)
{
  ...
  printf("%f %f %f %f %f\n",
	 ...
	 (*particle).position[2],(*particle).e_start,
	 180*asin(sqrt((*particle).sin2_alpha_start))/M_PI);
