...
int read_epot(char* el_filename) {
  ...
  if (pa_in == (FILE *)0)                 // disk error handling
	...
	ny = h.ny;
	nz = h.nz;
	if (nx > max_nx) printf("ERROR: read_epot: max_nx lower than nx!\n\n");
	else
	  {
	    for(z=0;z < h.nz; z++) {
	      for(y=0;y < h.ny; y++) {
  ...
  *splat = 0;
  ...
  for (comp = 0; comp <=2; comp++)
      ...
      if (pos_left[comp] < 0.) 
	{
      ...
      else pos_right[comp] = pos_left[comp] + 1;                      // corner 5
...
double get_epot2D(double *position_cm,int *splat)
{
  ...
  *splat = 0;
  ...
  if ((position[0]>nx)||(position[1]>ny)||(position[2]>nz)
      ||(pos_left[0]>nx)||(pos_left[1]>ny)||(pos_left[2]>nz)
      ||(pos_right[0]>nx)||(pos_right[1]>ny)||(pos_right[2]>nz))
    {
      return maxvoltage;                                        // watch for electrodes
    }
  else
...
{ // this function gives 3 dim potentials out of cylindric values from efield module
  ...
  if (ENABLE_EPOT == 1) // switch on and off retarding potential
      ...
      if (get_symmetry() == 0)
	{
      ...
      else e_pot = get_epot3D(position,&splat);     // use 3D potential array
...
{
  corner[0] = read_matrix_value(electric_potential_array, nx, ny, nz, 0, 
				(int)pos_left[0],
				(int)pos_left[1],
				(int)pos_left[2]);

  corner[1] = read_matrix_value(electric_potential_array, nx, ny, nz, 0, 
				(int)pos_right[0],
				(int)pos_left[1],
				(int)pos_left[2]);			   

  corner[2] = read_matrix_value(electric_potential_array, nx, ny, nz, 0, 
				(int)pos_right[0],
				(int)pos_right[1],
				(int)pos_left[2]);			   

  corner[3] = read_matrix_value(electric_potential_array, nx, ny, nz, 0, 
				(int)pos_left[0],
				(int)pos_right[1],
				(int)pos_left[2]);
}
      ...
      if (get_symmetry() == 0) // means cylindrical 2D symmetry
	{
	  ...
	  for (comp = 0; comp <=2; comp++)
	      ...
	      if (gpos_ll[comp] < 0.) 
		{
	      ...
	      else gpos_ur[comp] = gpos_ll[comp] + 1;    // lower right corner for + gpos
	      if (grid_pos[comp] < 0.) grid_pos[comp]= -grid_pos[comp]; // use positive values
	    }

	  if ((grid_pos[0]>nx)||(grid_pos[1]>ny)||(grid_pos[2]>nz)
	      ||(gpos_ll[0]>nx)||(gpos_ll[1]>ny)||(gpos_ll[2]>nz)
	      ||(gpos_ur[0]>nx)||(gpos_ur[1]>ny)||(gpos_ur[2]>nz))
	    {
