...
int read_mag_pa(char* mag_filename) {
  ...
  if (pa_in == (FILE *)0)                 // disk error handling
	...
	ny = h.ny;
	nz = h.nz;
	if (nx > max_nx) printf("ERROR: read_mag_pa: max_nx lower than nx!\n\n");
	else
	  {
	    for(z=0;z < h.nz; z++) {
	      for(y=0;y < h.ny; y++) {
...
{
  switch(USE_MAG_PA){
  case 0 : printf("ERROR: This version of ADIPARK does not support magnetic field calculation!\n");
  case 1 : get_bfield_std(position,bvec); break;
  case 2 : get_bfield_homogen(bvec); break;
  case 3 : b_toroid(position,bvec); break;
  case 4 : get_bfield_fg(position,bvec); break; 
  case 5: get_bfield_poles(position,bvec); break;
  default : printf("ERROR: In mag_pa_reader/get_bfield: PA use not set properly!\n");
   }
}
