  ...
  while (runfile >> repeat >> xi >> yi >> zi >> ekin >> thetai >> phii >> mass >> charge >> phasei) {
    ...
    if (transform) {
      ...
      for (Int_t j = 0; j < maxf; j++) {
        ...
        pc.outi = out[j][1];
        pc.fW_f = powerf[j];
        //appr energy spectral density E(f), total energy at f is integral of fJpHz 
        pc.fJpHz =powerf[j] * pow( tstep * US2S * max, 2);
        //time-integral square amplitude, total energy at bin f is fJ_f
        pc.fJ_f = powerf[j] * tstep * US2S * max;
        ...
        if (transNoise) {
          ...
          npc.outi = out_n[j][1];
          npc.fW_f = npowerf[j];
          npc.fJpHz = npowerf[j] * pow( tstep * US2S * max, 2);
          npc.fJ_f = npowerf[j] * tstep * US2S * max;
          nfftTree->Fill();
        }
      }
