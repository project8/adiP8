...
#define RGN_unit_conversion 1e-4
  ...
  if (RGN_pressure < 0.) {
    ...
    return RGN_pressure * RGN_unit_conversion / (RGN_k_Boltzmann * RGN_temp);
  }
...
{
  if ((dx_per_path_total >= dx_per_path_init) && (parameter.residual_gas_pressure >= 0.)) {
    return 1;
  } else {
    return 0;
  }
